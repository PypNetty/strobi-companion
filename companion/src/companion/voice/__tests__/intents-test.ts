...
describe('appearance intents', () => {
  ...
  it('rebuilds Strobi from spoken animal recipes and resets her own look', () => {
    expect(classifyIntent('je veux que tu ressembles à un lapin')).toEqual({
      type: 'shape',
      recipe: 'lapin',
    })
    expect(classifyIntent('ressembles à un lapin')).toEqual({ type: 'shape', recipe: 'lapin' })
    expect(classifyIntent('sois un chat')).toEqual({ type: 'shape', recipe: 'chat' })
    expect(classifyIntent('deviens un ours')).toEqual({ type: 'shape', recipe: 'ours' })
    expect(classifyIntent('un blob')).toEqual({ type: 'shape', recipe: 'blob' })
    expect(classifyIntent('comme un oiseau')).toEqual({ type: 'shape', recipe: 'oiseau' })
    expect(classifyIntent('en poisson')).toEqual({ type: 'shape', recipe: 'poisson' })
